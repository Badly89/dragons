...
class SudModel extends \yii\base\Model
{
    ...
    {
        return [
            [['action', 'id', 'aloneItem', 'active', 'itemId', 'items', 'title', 'short_text', 'full_text'], 'emptyValidate']
        ];
    }
        ...
        if (!$this->mode) {
            if (!Yii::$app->user->isGuest) {
                if (Users::findGroupById(Yii::$app->user->getId()) == 5) {
                    $this->items = Sud::getAllOrderByDesc();
                } else {
                    $this->items = Sud::getAllActiveOrderByDesc();
                }
            } else {
                $this->items = Sud::getAllActiveOrderByDesc();
            }
    ...
    public function processPostAction()
    {
        ...
        if ($this->action == "activateItem") {
            if (Users::findGroupById(Yii::$app->user->getId()) == 5) {
                $post = Sud::findOne(['id' => $this->itemId]);
                if ($post) {
                    $post->active = 1;
                    $post->save();
                }
    ...
    {
        $this->aloneItem = Sud::getOneById($this->id);
        if ($this->aloneItem) {
