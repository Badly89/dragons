...
class Ready extends \yii\base\Model {
    ...
    public function processAction() {
        ...
        if ($this->action == "done_chist" && $isDragon) {
            if ($this->dragonRights->chistota == 1) {
        ...
        if ($this->action == "done_katorga" && $isDragon) {
            if ($this->dragonRights->katorga == 1) {
                $this->z_actions = Actions::findActiveActionsByZid($this->z_id);
                $this->zayavka = Zayavka::findZayavkaById($this->z_id);
                if ($this->zayavka->active == 1) {
                    $this->new_action = new Actions();
    ...
    public function getReadyZayavki($dragonRights) {
        $cities = [];
        $this->zayavkiArray = [];
        ...
        if (sizeof($cities) > 0) {
            $commonCalculation = LastActiveActions::getReadyItems($cities);
        }
